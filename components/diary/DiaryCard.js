  ...
  const displayOthersPhotos = () => {
    if(editPhotos.length > 0) {
      ...
      for (let i = 1; i < editPhotos.length; i++) {
        othersPhotos.push(
          <View key={i} style={tw`flex items-center w-[50%] pt-[1rem] pr-[.5rem]`}>
            <Image source={{ uri: editPhotos[i] }} alt={`photo-${i}`} style={tw`rounded-[.5rem] w-full h-[8rem] mr-[.5rem]`} />
          </View>
        );
      }
  ...
  return (
    <View>
      {isLoading ? (
        ...
        <View>
          <View
            style={tw`bg-white/40 rounded-[.5rem] w-full p-[.5rem] mb-[.5rem]`}
          >
                  ...
                  <TouchableOpacity onPress={showAllImage} style={tw`h-[8rem] w-[8rem] rounded-[.5rem] ml-[.5rem] flex items-center justify-center`}>
                    ...
                    {editPhotos.length > 1 && (
                      <View style={tw`flex flex-row absolute items-center`}>
                        ...
                        />
                        <Text style={tw`text-black text-[2rem] opacity-60 font-bold`}>{editPhotos.length - 1}</Text>
                      </View>
                    )}
                  </TouchableOpacity>
                )}
            ...
            <View style={tw`flex flex-row w-full items-start pt-[.5rem]`}>
              {!edit && editPhotos.length > 0 && (
                <View style={[tw`w-[40%] rounded-[.5rem] min-h-[7rem]`, { height: textHeight }]}>
                  <Image source={{ uri: editPhotos[0] }} alt="photo" style={[tw`w-full rounded-[.5rem] min-h-[7rem]`, { height: textHeight }]} />
                </View>
              )}
              <TextInput
