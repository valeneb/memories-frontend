...
export default function Header({ isDairyActive, setIsDairyActive, id}) {
 ...
 const handleChange = () => {
    ...
    if(title !== travel.destination) {
        fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${title}&limit=1`)
        .then(response => response.json())
        .then(data => {
           infos= {latitude: data[0].lat, longitude: data[0].lon, destination: title, _id: id}
           updateInfos(infos);
        })
 ...
 return (
    <View style={tw`w-full`}>
        <TouchableHighlight onLongPress={() => setEdit(true)}>
            <View style={tw`bg-[#D8725B] pt-[2rem] px-[1rem] pb-[1rem] flex ${edit ? 'flex-col' : 'flex-row items-center justify-between'}`}>
                <TextInput value={title} onChangeText={(value) => setTitle(value)} editable={edit} style={tw`${edit ? 'my-[1rem] p-[.5rem] rounded-[.5rem] bg-[#D9D9D9] text-black w-[60%] text-[1.5rem]' : 'text-[#F2DDC2] text-[2rem]'}`} />
                <View style={tw`flex ${edit ? 'flex-col' : 'flex-row ml-[.5rem]'} items-center `}>
                    ...
                    <View style={tw`flex flex-row items-center ${edit ? 'w-full' : ''}`}>
                        <Text style={tw`text-[#F2DDC2] font-bold ${edit ? 'pr-[.5rem]' : 'px-[.5rem]'} text-[.875rem]`}>AU</Text>
                        {edit ? (
