 ...
 return (
   <View style={tw`w-full flex flex-col items-center justify-between h-full ${isKeyboardVisible ? 'absolute top-50' : ''}`}>
      <View style={tw`w-full flex flex-col items-center mt-[4rem]`}>
        ...
        {selectedImageUri ? (
            <TouchableOpacity onPress={handleAddImage} style={tw`w-[80%] h-[50%] mt-[1rem] rounded-[.5rem]`}>
               <Image source={{ uri: selectedImageUri }} style={tw`w-full h-full rounded-[.5rem]`} resizeMode="cover" />
            </TouchableOpacity>
        ) : (
